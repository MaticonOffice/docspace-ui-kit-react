...
import LightSmallRuLogo from "../../assets/logo/lightsmall.ru.svg";
...
const PortalLogo = ({ className, isResizable = false }: PortalLogoProps) => {
  ...
  const FallbackLogo = /^ru(?:[-_]|$)/i.test(getCookie(LANGUAGE) ?? "")
    ? LightSmallRuLogo
    : LightSmallLogo;
  ...
  if (isError) {
    return <FallbackLogo className={classNames("logo-wrapper", className)} />;
  }
